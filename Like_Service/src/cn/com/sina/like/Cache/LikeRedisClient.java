...
public class LikeRedisClient extends AbstractRedisClient {
	...
	public void setListLong(String key, List<Long> list) {
		...
		if (connection.setnx(LOCK_PERFIX + key, "1") != 0L) {
			//设置锁的过期时间
	...
	// long result = connection.setnx(LOCK_PERFIX + key,
	// Long.toString(System.currentTimeMillis()));
	// boolean getLockSuccess = false;
	// if (result == 0L) {
	// //获取锁失败，对锁进行超时检查
	// long currentLockStamp = Long.parseLong(connection.get(LOCK_PERFIX +
	// key));
	...
	// currentLockStamp = Long.parseLong(connection.getSet(
	// LOCK_PERFIX + key,
	// Long.toString(System.currentTimeMillis())));
	// if (System.currentTimeMillis() - currentLockStamp > EXPIRE_TIME) {
	// getLockSuccess = true;
